...
public class PlayerAttack : MonoBehaviour
{
    ...
    void AttackEnemies(Attack attack)
    {
        ...
        {
            // Detectar tanto enemigos como bosses
            if (col.CompareTag("Enemy") || col.CompareTag("Boss"))
            {
                // Intentar obtener el componente EnemigoBase
                ...
                // También intentar con BossHealth
                BossHealth boss = col.GetComponent<BossHealth>();
                if (boss != null)
                {
                    boss.RecibirDanio(attack.attackDamage);
                    Debug.Log($"¡Boss recibió daño! Vida actual: {boss.VidaActual}");
                }
            }
        }
    }
